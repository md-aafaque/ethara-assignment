...
export class TaskService {
  static async getTasks(filters: { teamId?: string; projectId?: string; status?: TaskStatus; assigneeId?: string }) {
  ...
  static async createTask(data: z.infer<typeof createTaskSchema>) {
    // Spec: Due date cannot be set in the past during creation
    if (data.dueDate && new Date(data.dueDate) < new Date()) {
      throw new Error('Due date cannot be in the past');
    }

    // Basic validation: ensure assignee is in the team
    if (data.assigneeId) {
  ...
  static async updateTask(id: string, userId: string, data: z.infer<typeof updateTaskSchema>) {
